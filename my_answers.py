...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        # delta_weights_i_h += np.dot(np.reshape(X, (X.shape[0], 1)), np.reshape(hidden_error_term, (hidden_error_term.shape[0], 1)).T )
        delta_weights_i_h += np.dot(hidden_error_term, inputs.T).T
        # print(delta_weights_i_h.shape)

        # Weight step (hidden to output)
        # delta_weights_h_o += np.dot(np.reshape(hidden_outputs, (hidden_outputs.shape[0], 1)), np.reshape(output_error_term, (output_error_term.shape[0], 1)))
        delta_weights_h_o += np.dot(output_error_term, hidden_outputs.T).T
